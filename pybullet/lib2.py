...
view_matrix = p.computeViewMatrixFromYawPitchRoll(
        cameraTargetPosition=[(max-min)/2+min,0.,-1]+offset,
        distance=5,
        yaw=90,
        pitch=-90,
        roll=0,
...
class PandaPushEnv:
    def __init__(self,gui=0,w=100,h=100):
        ...
        p.resetDebugVisualizerCamera(cameraDistance=1.5, cameraYaw=0, cameraPitch=-40, cameraTargetPosition=[0.55,-0.35,0.2])#the initial view of the environment
    ...
    def move(self,pos_goal,push_angle):
        ...
        while ((err_pos>threshold_pos) | (err_or>threshold_or) and count<350):
            pos_cur =self.observation[0:3]
            or_cur =  p.getEulerFromQuaternion(self.observation[3:7])[2]
            de = pos_goal-pos_cur
            do = push_angle-or_cur
    ...
    def step(self, action):
        ...
        self.changeObjPoseCamera(pos1+offset,or1,pos2+offset,or2)
    ...
    def reset(self,objectid1Pos,objectid2Pos,c1,c2):
        self.step_counter = 0
        self.prevPose = [0, 0, 0]
    ...
    def _get_state(self):
        ...
        self.observation = np.concatenate((state_robot_pos, state_robot_or,state_object_pos1, state_object_or1,state_object_pos2, state_object_or2),axis=0)
    ...
    def changeObjPoseCamera(self,position1,orientation1,position2,orientation2):
    ...
    def new_config(self,objectid1Pos,objectid2Pos):
        ...
        self.changeObjPoseCamera(objectid1Pos+offset,orientation,objectid2Pos+offset,orientation)
        p.stepSimulation()


    def action_fun(self,ob_id,idx2,idy2,c1,c2):
    ...
    def noaction_fun(self,class_img,c1,c2):
        idx1,idy1 = np.where(class_img==c1)
        pos1 = [sub_x[idx1],sub_y[idy1], 0.06]
        idx2,idy2 = np.where(class_img==c2)
        pos2 = [sub_x[idx2],sub_y[idy2], 0.06]

        self.changeObjPoseCamera(pos1+offset,p.getQuaternionFromEuler([0,0,0]),pos2+offset,p.getQuaternionFromEuler([0,0,0]))
        p.stepSimulation()
