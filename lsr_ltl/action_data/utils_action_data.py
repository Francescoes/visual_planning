...
def plot_decodings(img, dec_mean_original, dec_mean_samples, item, imgnum,
    ...
    for i in range(n_plots - 2):
        plt.subplot(2, 2, i+3)
        fig=plt.imshow(dec_mean_samples[i].cpu().squeeze().numpy().transpose(1, 2, 0))
        plt.title('decoded samples')
        fig.axes.get_xaxis().set_visible(False)
        fig.axes.get_yaxis().set_visible(False)
    plt.subplot(2, 2, 2)
    fig=plt.imshow(dec_mean_original.cpu().squeeze().numpy().transpose(1, 2, 0))
    plt.title('decoded original')
    fig.axes.get_xaxis().set_visible(False)
    fig.axes.get_yaxis().set_visible(False)

    plt.subplot(2, 2, 1)
    fig=plt.imshow(img.cpu().squeeze().numpy().transpose(1, 2, 0))
    plt.title('original')
    fig.axes.get_xaxis().set_visible(False)
    fig.axes.get_yaxis().set_visible(False)
