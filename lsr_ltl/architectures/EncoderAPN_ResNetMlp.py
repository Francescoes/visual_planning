...
class ResBlock(nn.Module):
    def __init__(self, in_channels, out_channels, depth=2, kernel_size=3, dropout=0):
        ...
        for d in range(depth):
            ...
            self.res_layer.add_module('P_res_arelu'+str(d), PrintShape('Input to  res_conv'))
            ...
            self.res_layer.add_module('P_res_conv'+str(d), PrintShape('Output of res_conv'))
...
class FCResBlock(nn.Module):
    def __init__(self, in_dim, out_dim, depth=2, dropout=0):
        ...
        for d in range(depth):
            ...
            self.fcres_layer.add_module('P_fcres_relu'+str(d), PrintShape('Input to  fcres_lin'))
            ...
            self.fcres_layer.add_module('P_fcres_ali'+str(d), PrintShape('Output of fcres_lin'))
...
class EncoderAPNet_ResNetMlp(nn.Module):
    """
    ...
    def __init__(self, opt, trained_params=None):
        ...
        for d in range(self.num_scale_blocks):
            self.enc_conv.add_module('enc_scale' + str(d),
                    ScaleBlock(self.out_channels, self.out_channels, 
                               ...
                               self.kernel_size, self.encoder_dropout))
            
            if d != self.num_scale_blocks - 1:
                in_channels = self.out_channels
                self.out_channels *= 2
                self.enc_conv.add_module('P_enc_bdownscale'+str(d), PrintShape('Input to  enc_downscale'))
        ...
        self.enc_conv.add_module('P_enc_bpool', PrintShape('Input to  enc_avgpool'))
        self.enc_conv.add_module('enc_avgpool', nn.AvgPool2d(3))
        self.enc_conv.add_module('P_enc_apool', PrintShape('Input to  enc_flatten'))
        self.enc_conv.add_module('enc_flatten', ConvToLin())
        self.enc_conv.add_module('P_enc_bfcs', PrintShape('Input to  enc_fcscale'))
        self.enc_conv.add_module('enc_fcscale', FCScaleBlock(
                self.fc_dim, self.fc_dim, 1, self.depth_per_block, self.encoder_dropout))
        self.enc_conv.add_module('P_enc_afcs', PrintShape('Output of enc_fcscale'))
