...
class AE_TinyResNet(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.decoder_fn = self.decoder_mean
...
#        scales, dims = self.get_decoders_shape()
    ...
    def decoder(self, z):
        ...
        """
        return self.decoder_mean(z) #if self.learn_dec_logvar else self.decoder_mean(z)
    ...
    def forward(self, x, sample_latent=False, latent_code=False):
        ...
        #     out_mean= self.decoder_fn(z)
        #     return out_mean, latent_mean
        out_mean= self.decoder_fn(latent_mean)
        dummy_dec = 0
        dummy_enc = 0
        return out_mean, latent_mean #out_mean, dummy_dec, latent_mean, dummy_enc
