...
class APNet(nn.Module):
    ...
    def __init__(self, opt, trained_params=None):
        ...
        print(len(self.shared_imgnet_dims))
    ...
    def forward_and_transform(self, img1, img2):
        ...
        out_descaled[:, 2] = np.around(out_denorm_height)
        return out_descaled
...
if __name__ == '__main__':
    ...
    opt = {
            'device': 'cpu',
            'shared_imgnet_dims': [12, 64],
            'separated_imgnet_dims': [64, 16],
            'shared_reprnet_dims': [32, 16, 5],   
            'dropout': 0.2,
    ...
    print(img1.shape)
    print(out.shape)
    print((1274*size)/count_parameters(net))
    print('\n * ---')
