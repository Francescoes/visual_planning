...
#plot path
def generate_sequence2(length,f_start,f_goal,distance_type,image_save_name,device,vae_algorithm,G,k):
    ...
    # if there is no path return [] so that this start and goal pair is skipped
    if Paths_number==0:
        return []
    ...
    if save_img:
        ...
        cv2.imwrite(image_save_name+"_"+str(k)+"_1_1.png",np.concatenate([combo_img_vp[x] for x in range(len(combo_img_vp)-1)],axis=0))
...
def generate_sequence(length,f_start,f_goal,distance_type,image_save_name,device,vae_algorithm,G,k):
    ...
    for idx in range(Paths_number):
        ...
        if save_img:
            path_img2=[]
            path_img2.append(f_start)
            path_img2.append(buffer_img_v)
        ...
        for l in path:
            ...
            if save_img:
                ...
                img_rec_cv=img_rec[0].detach().permute(1,2,0).cpu().numpy()
                path_img2.append(img_rec_cv)
                path_img2.append(buffer_img_tiny)

        if save_img:
            path_img2 = path_img2[:-1]
            path_img2.append(buffer_img_v)
            path_img2.append(f_goal)
        ...
        if not(check1==check2):
            ...
            if save_img:
                ...
                cv2.imwrite(image_save_name+"_"+str(k)+"_1_"+str(check1)+".png",np.concatenate([combo_img_vp[x] for x in range(len(combo_img_vp)-1)],axis=0))
                ...
                combo_img_vp=[]
                combo_img_vp.append(np.concatenate([path_img2[x] for x in range(len(path_img2))],axis=1))
                buffer_img_h=np.ones((30,combo_img_vp[0].shape[1],3),np.uint8)
                combo_img_vp.append(buffer_img_h)

                cv2.imwrite(image_save_name+"_"+str(k)+"_2_"+str(check2)+".png",np.concatenate([combo_img_vp[x] for x in range(len(combo_img_vp)-1)],axis=0))



            return check1,path1,check2,path2
...
def main():
    ...
    while k<N:
        ...
        while True:
            ...
            path1 = generate_sequence2(length,i_start/255.,i_goal/255.,distance_type,image_save_name,device,vae_algorithm,G,k)
            while len(path1)==0:
                start_idx=random.randint(0,len(dataset2)-1)
                goal_idx=random.randint(0,len(dataset2)-1)
                if not([start_idx,goal_idx] in already_seen_pair2):
    ...
    with open("./datasets_lstm/X_a.pkl", 'wb') as f:
        pickle.dump(X, f)
    with open("./datasets_lstm/Y_a.pkl", 'wb') as f:
        pickle.dump(Y, f)
