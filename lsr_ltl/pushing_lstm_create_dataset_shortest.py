...
def generate_sequence(length,f_start,f_goal,distance_type,image_save_name,device,vae_algorithm,G,k,X,Y):
    ...
    try:
        l = nx.shortest_path_length(G, source=c1_close_idx, target=c2_close_idx)
        print ("Length: ", l)
        if l > length-1:
            return 0
...
def main():
    ...
    with open("./datasets_lstm/X_a_sh.pkl", 'wb') as f:
        pickle.dump(X, f)
    with open("./datasets_lstm/Y_a_sh.pkl", 'wb') as f:
        pickle.dump(Y, f)
