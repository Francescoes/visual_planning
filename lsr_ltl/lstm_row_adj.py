...
X_pickle = pickle.load( open( "./datasets_lstm/X_ra.pkl", "rb" ) )
Y_pickle = pickle.load( open( "./datasets_lstm/Y_ra.pkl", "rb" ) )
...
for k in range(K):
    ...
    for inputs, labels, lens in test_loader:
        ...
        output_r, h_r = model_r(inputs, h, lens)
        pred_a = torch.round(output_a.squeeze()) #rounds the output to 0/1
        pred_r = torch.round(output_r.squeeze()) #rounds the output to 0/1

        # print(torch.max(pred_A, pred_B))
        # exit(0)

        pred_both = torch.mul(pred_a ,  pred_r)
        ...
        correct_tensor = pred_both.eq(labels.float().view_as(pred_both))
        correct = np.squeeze(correct_tensor.cpu().numpy())
        num_correct += np.sum(correct)


        for k in range(len(pred_both)):
            if labels[k]==0:
                num_neg+=1
            else:
                num_pos+=1

            if pred_both[k]==labels[k]:
                if pred_both[k]==0:
                        num_correct_neg+=1
                else:
                    num_correct_pos+=1
