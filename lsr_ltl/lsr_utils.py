...
def build_lsr(latent_map_file,epsilon,distance_type,graph_name, config_file,checkpoint_file,min_edge_w=0,min_node_m=0, directed_graph = False,  hasclasses=False, verbose = False, save_graph = False):
    ...
    for latent_pair in latent_map:
        ...
        if action==1:
            ...
            G1.add_node(c_idx,pos=z_pos_c1,_class=latent_pair[4])
            Z_all.add(c_idx)
            c_idx=G1.number_of_nodes()
            G1.add_node(c_idx,pos=z_pos_c2,_class=latent_pair[5])
            Z_all.add(c_idx)
    ...
    nx.write_gpickle(G1, "./check_eq/G1.pkl")
    ...
    while len(Z_all) >0:
        ...
        with open("./check_eq/Z_sys_is.pkl", 'wb') as f:
            pickle.dump(Z_sys_is, f)
...
#         W_z_c_pos=np.mean(w_pos_all,axis=0)
        W_z_c_pos_mean=np.mean(w_pos_all,axis=0)

        ################
        #find the closest nodes
#         W_z_c_pos = G1.nodes[w]['pos'] # just o initialize with a valid value
        min_distance_w=np.Inf

        for w in W_z:
            distance_w=np.linalg.norm(w-W_z_c_pos_mean, ord=distance_type)
            if distance_w<min_distance_w:
                min_distance_w=distance_w
                W_z_c_pos = G1.nodes[w]['pos']
                W_z_c_class = G1.nodes[w]['_class']
        ################
        ...
        G2.add_node(c_idx,pos=W_z_c_pos,image=img_rec_cv,W_z=W_z,w_pos_all=w_pos_all,_class=W_z_c_class)
...
def lable_latent_space(config_file,checkpoint_file,output_file,dataset_name):
    ...
    for i in range(len(dataset)):
        ...
        for i in range(z.size()[0]):
            ...
            latent_map.append((z_np,z2_np,a,a_lambda,class_1,class_2))
...
#Label latent space given VAE and dataset
def lable_latent_space_new_format(config_file,checkpoint_file,output_file,dataset_name):
