...
def generate_sequence(length,f_start,f_goal,distance_type,image_save_name,device,vae_algorithm,G,k):
    ...
    if ltl == "A":
        check1 = check_A(path,G)
    if ltl == "B":
        check1 = check_B(path,G)
    if ltl == "C":
        check1 = check_C(path,G)
    if ltl == "ABC":
        check1 = (check_A(path,G) or check_B(path,G)) and check_C(path,G)
...
def main():
    ...
    image_save_name="./Img_lstm_ABC/LSTM_img"
    ...
    while k<N:
        ...
        check1,path1 = generate_sequence(length,i_start/255.,i_goal/255.,distance_type,image_save_name,device,vae_algorithm,G,k)
    ...
    with open("./datasets_lstm_ABC/X"+ltl+".pkl", 'wb') as f:
        pickle.dump(X, f)
    with open("./datasets_lstm_ABC/Y"+ltl+".pkl", 'wb') as f:
        pickle.dump(Y, f)
