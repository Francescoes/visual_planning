...
# -*- coding: utf-8 -*-
"""
Created on Sun Mar 22 10:45:40 2020
...
class APN_folding_dev(APN_Algorithm):
    ...
    def load_norm_param_d_fromAFile_archived(self, path_to_norm_param_d, random_seed=''):
        """Loads the dict with normalisation parameters but from a particular file"""
        with open(path_to_norm_param_d, 'rb') as f:
            self.norm_param_d = pickle.load(f)
...
#        TODO: shuffle, sampler
    ...
    def score_model(
        ...
        if load_checkpoint:
            ...
            path_to_trained_APNmodel = './models/{0}/apnet_bestModelAll.pt'.format(model_name)
...
#            pad_right = abs(int(self.data_max)) - self.img_size
    ...
    def score_model_newHeldout(
            self, model_name, path_to_valid_dataset,  path_to_result_file, 
            random_seed, load_checkpoint=False, path_to_chpnt='', suffix='', 
            data_min=None, data_max=None):
        """Scores a trained model on the test set, pixel coordinates not 
        scaled to 0,1."""
        ...
        assert(not self.model.training)
        if not self.img_size:
            self.img_size = raw_valid_data[0][0].shape[0]
        ...
        # Filter out action pairs and preprocess
        valid_data = []
        for t in raw_valid_data:
            if t[2] == 1:
                img1 = torch.tensor(t[0]/255.).float().permute(2, 0, 1)
                img2 = torch.tensor(t[1]/255).float().permute(2, 0, 1)
                input_uv = t[3][0].reshape(-1, 1)        
                input_h = np.array(int(t[3][2])).reshape(-1, 1)
                output_uv = t[3][1].reshape(-1, 1)
                coords_array = np.concatenate([input_uv, input_h, output_uv]).astype('float32')
                
                valid_data.append((img1, img2, coords_array))
                
        # Compute the error
        for img1, img2, pixel_coords in valid_data:

            n_points += 1

            # Rescale the params
            height = pixel_coords[2]
            coords_scaled = (pixel_coords - self.data_min)/(self.data_max - self.data_min)
            coords_scaled[2] = height # don't rescale the height
        
            assert(np.all(coords_scaled) >= 0. and np.all(coords_scaled) <= 1.)
            coords_unnorm = torch.from_numpy(coords_scaled).squeeze()  
            ...
            # Descale and denormalise back to pixels
            pred_coords_denorm = self.denormalise(pred_coords)
            pred_coords_descaled = self.descale_coords(pred_coords_denorm)
            
            pred_height = np.round(pred_coords_denorm[:, 2])
            pred_coords_descaled[:, 2] = pred_height
            pred_coords_descaled_t = torch.from_numpy(pred_coords_descaled).to(self.device).float()
            pixel_coords_t = torch.from_numpy(pixel_coords.transpose(1, 0)).to(self.device).float()
            ...
            # print('\ndenormalised')
            # print(pred_coords_denorm, pred_coords_denorm.shape)
            
            # print('\ndescaled')
            # print(pred_coords_descaled, pred_coords_descaled.shape)
            
            # print(pixel_coords_t, pixel_coords_t.shape)
            ## print(pred_coords_descaled_t, pred_coords_descaled_t.shape)
            
            # Compute the mse loss on pixels and log the resutls
            (the_loss, inputXloss, inputYloss, inputHloss, outputXloss, 
                 outputYloss) = self.compute_loss(pred_coords_descaled_t, pixel_coords_t) 
            ...
            if (not os.path.isdir('{0}/NewHeldout'.format(self.opt['exp_dir']))):
                    os.makedirs('{0}/NewHeldout'.format(self.opt['exp_dir']))
            
            # --- Separately save cases with high error
            if (inputXloss.item() > 20 or inputYloss.item() > 20 or 
                outputXloss.item() > 20 or outputYloss.item() > 20):
                failure_dir = '{0}/NewHeldout/best_model_failures'.format(
                    self.opt['exp_dir'])
                if (not os.path.isdir(failure_dir)):
                    os.makedirs(failure_dir)
            ...
            if n_points % 5 == 0:
                self.plot_prediction(
                        enc_mean1, enc_mean2, pred_coords, coords,
                        split='valid' + str(n_points), n_subplots=1, 
                        new_save_path = '{0}/NewHeldout/'.format(self.opt['exp_dir'])
                        )
            
            # --- Compare the distribution of predictions vs ground truth
            # Append for histogram plots
            pred_coord_list.append(pred_coords_descaled.reshape(-1, 1))
            actual_coord_list.append(pixel_coords.reshape(-1, 1))
            
            pred_pick_xy = pred_coords_descaled[0][:2] + abs(int(self.data_min))
            actual_pick_xy = coords[0][:2] + abs(int(self.data_min))
            pred_place_xy = pred_coords_descaled[0][3:] + abs(int(self.data_min))
            actual_place_xy = coords[0][3:] + abs(int(self.data_min))
...
#            pad_right = abs(int(self.data_max)) - self.img_size
        ...
        for i in range(len(picks_list)):            
            ...
            picks_image = cv2.circle(picks_image, picks_list[i][1], 15, (144, 238, 144), 4)
        fig=plt.imshow(picks_image)
        plt.savefig('./models/{0}/NewHeldout/{1}'.format(model_name, 'All_valid_picks' + suffix))
        plt.clf()
        plt.close()
        ...
        for i in range(len(picks_list)):            
            ...
            picks_image = cv2.circle(picks_image, (picks_list[i][1][0], 120), 15, (144, 238, 144), 4)
        fig=plt.imshow(picks_image)
        plt.savefig('./models/{0}/NewHeldout/{1}'.format(model_name, 'All_valid_picks_x' + suffix))
        plt.clf()
        plt.close()
        ...
        for i in range(len(picks_list)):            
            ...
            picks_image = cv2.circle(picks_image, (120, picks_list[i][1][1]), 15, (144, 238, 144), 4)
        fig=plt.imshow(picks_image)
        plt.savefig('./models/{0}/NewHeldout/{1}'.format(model_name, 'All_valid_picks_y' + suffix))
        plt.clf()
        plt.close()
        ...
        for i in range(len(places_list)):            
            ...
            places_image = cv2.circle(places_image, places_list[i][1], 15, (144, 238, 144), 4)
        fig=plt.imshow(places_image)
        plt.savefig('./models/{0}/NewHeldout/{1}'.format(model_name, 'All_valid_places' + suffix))
        plt.clf()
        plt.close()
        ...
        for i in range(len(picks_list)):            
            ...
            places_image = cv2.circle(places_image, (places_list[i][1][0], 120), 15, (144, 238, 144), 4)
        fig=plt.imshow(places_image)
        plt.savefig('./models/{0}/NewHeldout/{1}'.format(model_name, 'All_valid_places_x' + suffix))
        plt.clf()
        plt.close()
        ...
        for i in range(len(picks_list)):            
            ...
            places_image = cv2.circle(places_image, (120, places_list[i][1][1]), 15, (144, 238, 144), 4)
        fig=plt.imshow(places_image)
        plt.savefig('./models/{0}/NewHeldout/{1}'.format(model_name, 'All_valid_places_y' + suffix))
        plt.clf()
        plt.close()
        ...
        for i in range(len(places_list)):            
            ...
            all_image = cv2.circle(all_image, picks_list[i][1], 15, (144, 238, 144), 4)
        fig=plt.imshow(all_image)
        plt.savefig('./models/{0}/NewHeldout/{1}'.format(model_name, 'All_valid_everything' + suffix))
        plt.clf()
        plt.close()
        ...
        plt.title('pickX')
        plt.show()
        plt.savefig('./models/{0}/NewHeldout/{1}'.format(model_name, 'AllValidHist_picks_x' + suffix))
        plt.clf()
        plt.close()
        ...
        plt.title('pickY')
        plt.show()
        plt.savefig('./models/{0}/NewHeldout/{1}'.format(model_name, 'AllValidHist_picks_y' + suffix))
        plt.clf()
        plt.close()        
        ...
        plt.title('placeX')
        plt.show()
        plt.savefig('./models/{0}/NewHeldout/{1}'.format(model_name, 'AllValidHist_place_x' + suffix))
        plt.clf()
        plt.close()
        ...
        plt.title('placeY')
        plt.show()
        plt.savefig('./models/{0}/NewHeldout/{1}'.format(model_name, 'AllValidHist_place_y' + suffix))
        plt.clf()
        plt.close()
        # ----- END OF DISTRIBUTION PLOTS 
    ...
    def score_model_fromAFile_archived(self, model_name, path_to_valid_dataset, 
                              path_to_result_file, load_checkpoint=False, suffix=''):
        # Load the data        
        ...
        if type(valid_data) == dict:
            ...
            threshold_min, threshold_max = -100., 400.
        self.data_min = threshold_min
        self.data_max = threshold_max
        print(' *- Chosen thresholds ', threshold_min, threshold_max)
        ...
        for item in valid_data:
            ...
            coords = torch.from_numpy(
                    (coords_array_scaled - self.norm_mean.reshape(-1, 1))/self.norm_std.reshape(-1, 1))
            coords = coords.permute(1, 0).float().to(self.device)

            # VAE forward pass
            img1 = torch.from_numpy(img1.transpose(2, 0, 1)).to(self.device).unsqueeze(0).float()
            img2 = torch.from_numpy(img2.transpose(2, 0, 1)).to(self.device).unsqueeze(0).float()        
