...
class LBN_Algorithm():
    ...
    def plot_test_images(self, valid_dataset):
        """Plots sthe LBN predictions on a subset of test set."""
        self.model.eval()
    ...
    def plot_learning_curve(self):
        """Plots train and validation learning curves of the LBN training."""
    ...
    def compute_loss(self, pred_labels, labels):
        """Computes the loss on the training batch given the criterion."""
        batch_loss = nn.MSELoss(reduction='none')(pred_labels, labels) # (batch, 2)
        # print('\n\n In compute_loss: \n')
        # print('\n', batch_loss.shape, '\n', batch_loss)
        per_feat_loss = torch.mean(batch_loss, dim=0) # (2)
        # print('\n', per_feat_loss.shape, '\n', per_feat_loss)
        the_loss = torch.sum(per_feat_loss)
        # print('\n', the_loss.shape, '\n', the_loss)
        # print('\n End of compute_loss')
        return (the_loss, per_feat_loss[0], per_feat_loss[1])
    ...
    def compute_test_loss(self, valid_dataset):
        ...
        losses = np.zeros(4)
        for batch_idx, (img1, img2, labels) in enumerate(valid_dataloader):
            img1 = img1.to(self.device)
            img2 = img2.to(self.device)
        ...
        # if (self.current_epoch + 1) % self.snapshot == 0:
            # self.plot_prediction(img1, img2, pred_labels, labels, split='test')
    ...
    def train(self, train_dataset, test_dataset, num_workers=0, chpnt_path=''):
        """
        Trains an LBN model with given the hyperparameters.
    ...
    def save_logs(self, train_dataset, test_dataset):
        ...
        with open(log_filename, 'w') as f:
            ...
            f.writelines(list(map(
                    ...
                    epoch_losses[:, 0], valid_losses[:, 0], epoch_losses[:, -1], self.epoch_times)))

            f.write('*- Train/validation inputX Loss\n')
            f.writelines(list(map(
                    lambda t, v, e, time: '{0:>3}Epoch {3:.0f} [{4}] {1:.3f}/{2:.3f}\n'.format('', t, v, e, time),
                    epoch_losses[:, 1], valid_losses[:, 1], epoch_losses[:, -1], self.epoch_times)))

            f.write('*- Train/validation inputY Loss\n')
            f.writelines(list(map(
                    lambda t, v, e, time: '{0:>3}Epoch {3:.0f} [{4}] {1:.3f}/{2:.3f}\n'.format('', t, v, e, time),
                    epoch_losses[:, 2], valid_losses[:, 2], epoch_losses[:, -1], self.epoch_times)))
...
if __name__ == '__main__':
    class LBNDataset(data.Dataset):
        ...
        def __getitem__(self, index):
            img1, img2, labels = self.data[index]
            return img1, img2, labels

        def __len__(self):
            return len(self.data)

    train_dataset = LBNDataset('action_shirt_teleop_BigResNet_md30_wd1_ld128_is256_ipyes', 'train')
    test_dataset = LBNDataset('action_shirt_teleop_BigResNet_md30_wd1_ld128_is256_ipyes', 'test')


    opt = {
        'filename': 'lbnet',
        'exp_dir': 'LBNet_TESTINGTBD',
        'vae_name': 'ShirtTeleop_BigResNet_md30_wd1_ld128_is256_ipyes',
        'model_module': 'LBN_Mlp',
        'model_class': 'LBNet_Mlp_1Head_2Bodies_1Tail',
