...
class APN_Algorithm():
    ...
    def save_logs(self, train_dataset, test_dataset):
        ...
        with open(log_filename, 'w') as f:
            ...
            f.writelines(list(map(
                    lambda t, v, e, time: '{0:>3}Epoch {3:.0f} [{4}] {1:.3f}/{2:.3f}\n'.format('', t, v, e, time),
                    epoch_losses[:, 3], valid_losses[:, 3], epoch_losses[:, -1], self.epoch_times)))

            f.write('*- Train/validation outputX Loss\n')
            f.writelines(list(map(
                    lambda t, v, e, time: '{0:>3}Epoch {3:.0f} [{4}] {1:.3f}/{2:.3f}\n'.format('', t, v, e, time),
                    epoch_losses[:, 4], valid_losses[:, 4], epoch_losses[:, -1], self.epoch_times)))

            f.write('*- Train/validation outputY Loss\n')
            f.writelines(list(map(
                    lambda t, v, e, time: '{0:>3}Epoch {3:.0f} [{4}] {1:.3f}/{2:.3f}\n'.format('', t, v, e, time),
                    epoch_losses[:, 5], valid_losses[:, 5], epoch_losses[:, -1], self.epoch_times)))
